...
/* ================= PALETTE ================= */
// bg:        #161c17  (deep forest, not pitch black)
// bg-alt:    #1c241d  (slightly lighter for alternating sections)
// card:      #232b24  (card surfaces)
...
export default function Home() {
  ...
  return (
    <div style={{ background: "#161c17", color: "#e8ede8", minHeight: "100vh" }}>
      ...
      <section style={{
        ...
        background: "linear-gradient(135deg, #161c17 0%, #1a2a1b 50%, #161c17 100%)",
        gap: 48,
      }}>
        {/* Subtle radial glow */}
        <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", position: "relative" }}>
          ...
          <motion.p
            ...
            style={{ marginTop: 24, fontSize: "clamp(15px, 1.5vw, 18px)", color: "#8aa88d", maxWidth: 480, fontWeight: 400, lineHeight: 1.75 }}
          >
          ...
          >
            ...
            <a
              href="#how"
              style={{
                background: "rgba(255,255,255,0.06)", color: "#e8ede8",
                padding: "16px 36px", borderRadius: 12,
        ...
        >
          ...
          >
            ...
            {/* Phone Content */}
            <div style={{ padding: "16px 14px", display: "flex", flexDirection: "column", gap: 10, background: "#1e2720" }}>
              {/* QR Card */}
              <div style={{
                background: "#2a3530", borderRadius: 16, padding: "20px 16px",
                ...
                <span style={{ fontSize: 12, color: "#8aa88d", fontWeight: 500 }}>Scan to recover item</span>
              </div>

              {/* Notification Card */}
              <div style={{
                background: "#2a3530", borderRadius: 14, padding: "12px 14px",
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#e8ede8", lineHeight: 1.3 }}>Someone found your Laptop!</div>
                  <div style={{ fontSize: 11, color: "#8aa88d", marginTop: 2 }}>Reply here to chat anonymously</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7, padding: "4px 2px" }}>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    background: m.type === "out" ? "#1db954" : "#2a3530",
      ...
      <div style={{ display: "flex", flexWrap: "wrap", borderTop: "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)", gap: 1, background: "rgba(255,255,255,0.03)" }}>
        {stats.map((s, i) => (
          <div key={i} style={{ flex: "1 1 180px", padding: "48px 32px", textAlign: "center", background: "#161c17" }}>
            <div style={{ fontFamily: "Syne, sans-serif", fontSize: 48, fontWeight: 800, lineHeight: 1, color: "#e8ede8" }}>
              {s.num.replace(/\d+/, (n) => `${n}`).split('').map((c, j) =>
                /\d/.test(c) ? <span key={j} style={{ color: "#e8ede8" }}>{c}</span> : <span key={j} style={{ color: "#1db954" }}>{c}</span>
              )}
            </div>
            <div style={{ marginTop: 8, fontSize: 14, color: "#8aa88d" }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <section style={{ padding: "120px 48px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "clamp(32px, 4vw, 52px)", fontWeight: 800, letterSpacing: -1, lineHeight: 1.1, marginBottom: 64, color: "#e8ede8" }}>
          Shop Tags
        </h2>
          ...
          {products.map((p, i) => (
            ...
            >
              <Link href={`/product/${p.id}`} style={{ textDecoration: "none", display: "block" }}>
                <div style={{ padding: "28px 28px 0", background: "#1c241d", borderRadius: "16px 16px 0 0" }}>
                  <Image
                    src={p.image}
                    alt={p.name}
                    width={220}
                ...
                <div style={{ padding: "20px 24px 8px" }}>
                  <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 17, color: "#e8ede8", marginBottom: 6 }}>
                    {p.name}
                  </h3>
                  <p style={{ fontSize: 20, fontWeight: 700, color: "#1db954", marginBottom: 4 }}>
                    ₦{p.price.toLocaleString()}
                  </p>
                  <p style={{ fontSize: 13, color: "#8aa88d" }}>Smart QR recovery tag</p>
                </div>
              </Link>
        ...
        <div style={{ textAlign: "center", marginTop: 48 }}>
          <Link
            href="/shop"
            style={{
              border: "1.5px solid rgba(255,255,255,0.15)", color: "#e8ede8",
      ...
      {/* ===================== PHONE DEMO ===================== */}
      <div style={{ background: "#1c241d", borderTop: "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{
          ...
          >
            ...
            <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "clamp(28px, 4vw, 48px)", fontWeight: 800, letterSpacing: -1, lineHeight: 1.1, marginBottom: 20, color: "#e8ede8" }}>
              From scan to connected in seconds
            </h2>
            <p style={{ color: "#8aa88d", fontSize: 16, lineHeight: 1.8, marginBottom: 28 }}>
            ...
            <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                <li key={i} style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 15, color: "#8aa88d" }}>
                  <span style={{
          ...
          >
            <div style={{
              width: 260, background: "#232b24",
              borderRadius: 40, border: "2px solid rgba(255,255,255,0.08)",
              padding: "20px 16px",
              boxShadow: "0 32px 70px rgba(0,0,0,0.4)",
            }}>
              <div style={{ width: 80, height: 20, background: "#1a2118", borderRadius: 20, margin: "0 auto 16px", border: "2px solid rgba(255,255,255,0.05)" }} />
              <div style={{ background: "#1a2520", borderRadius: 24, padding: 20, minHeight: 380, display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  ...
                  <motion.div
                    ...
                    style={{
                      ...
                      background: m.type === "out" ? "#1db954" : m.type === "sys" ? "rgba(29,185,84,0.1)" : "#2a3530",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24 }}>
          {features.map((f, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              style={{
                background: "#232b24", border: "1px solid rgba(255,255,255,0.07)",
            ...
            >
              ...
              <h3 style={{ fontFamily: "Syne, sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 10, color: "#e8ede8" }}>
                {f.title}
              </h3>
              <p style={{ fontSize: 14, color: "#8aa88d", lineHeight: 1.7 }}>{f.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      {/* ===================== ABOUT ===================== */}
      <section id="about" style={{ background: "#1c241d", borderTop: "1px solid rgba(255,255,255,0.06)", padding: "120px 48px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 80, alignItems: "center" }}>
          ...
          >
            ...
            <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "clamp(32px, 4vw, 52px)", fontWeight: 800, letterSpacing: -1, lineHeight: 1.1, marginBottom: 24, color: "#e8ede8" }}>
              Meet Find<span style={{ color: "#1db954" }}>Me</span>
            </h2>
            <p style={{ color: "#8aa88d", fontSize: 16, lineHeight: 1.8, marginBottom: 16 }}>
              FindMe connects you instantly with anyone who finds your lost items. No apps, no stress — just scan and chat.
            </p>
            <p style={{ color: "#8aa88d", fontSize: 16, lineHeight: 1.8, marginBottom: 36 }}>
      ...
      {/* ===================== FINAL CTA ===================== */}
      <section style={{
        background: "linear-gradient(135deg, #1a2d1c, #162318)",
        ...
        >
          ...
          <p style={{ color: "#8aa88d", fontSize: 18, maxWidth: 480, margin: "0 auto 40px" }}>
