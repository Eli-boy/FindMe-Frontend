...
export default function OrderSuccessClient() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await fetch(`/api/paystack/verify?ref=${ref}`);
        const data = await res.json();

        if (data.paid && !hasFiredPurchase.current) {
      ...
      >
        {status === "loading" && (
          <>
            ...
            <p style={{ color: "#4a7a5a", fontSize: 15, lineHeight: 1.7 }}>
          ...
          <>
            ...
            {orderNumber && (
              <p style={{ color: "#4a7a5a", fontSize: 15, marginBottom: 8 }}>
                Order{" "}
                <strong style={{ color: DARK }}>#{orderNumber}</strong> is
                confirmed and paid.
              </p>
            )}
            <p
        ...
        {status === "pending" && (
          <>
            ...
            {orderNumber && (
              <p style={{ color: "#4a7a5a", fontSize: 15, marginBottom: 8 }}>
                Order{" "}
                <strong style={{ color: DARK }}>#{orderNumber}</strong> has
                been created.
              </p>
            )}
            <p
            ...
            >
              ...
              Check your email for confirmation or contact us on WhatsApp.
            </p>
            <div
              style={{
                display: "flex",
