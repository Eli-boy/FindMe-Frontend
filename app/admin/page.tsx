...
const api = (action: string, extra = {}) =>
  fetch("/api/admin", {
    ...
    body: JSON.stringify({
      action,
      password: sessionStorage.getItem("fm_admin_pass") || "findme2026",
      ...extra,
    }),
  }).then((r) => r.json());
...
export default function AdminDashboard() {
  ...
  const [openCouponMenu, setOpenCouponMenu] = useState<number | null>(null);
  ...
  const handleCreateManualOrder = async () => {
    ...
    setSaving(true);
    const total = manualForm.items.reduce(
  ...
  const generateAndExport = async () => {
    if (printCount < 1 || printCount > 300) {
      showToast("Enter a number between 1 and 300");
      return;
    }
    ...
    try {
      ...
      const allCodes = data.codes.map((code: string) => {
        ...
        return {
          ...
          scan_url: scanUrl,
          qr_api_url:
            "https://api.qrserver.com/v1/create-qr-code/?size=600x600&ecc=H&data=" +
            encodeURIComponent(scanUrl),
        };
      });
      const csv = [
        "code,scan_url,qr_preview_url",
        ...allCodes.map(
          (c: any) => c.code + "," + c.scan_url + "," + c.qr_api_url
        ),
      ].join("\n");
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download =
      "findme-qr-codes-" + new Date().toISOString().slice(0, 10) + ".csv";
    a.click();
  ...
  const createCoupon = async () => {
    ...
    const res = await api("create_coupon", {
      coupon: {
    ...
    if (status === "packed") {
      try {
        ...
        if (data.success) {
          ...
          showToast(
            "Status updated, but email failed: " + (data.error || "unknown")
          );
        }
      } catch {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p style={{ color: "#4a7a5a", fontSize: 13, marginBottom: 32 }}>
            FindMe Admin Dashboard
          </p>
          <input
            type="password"
          ...
          <button
            onClick={login}
            style={{
              ...btn({ width: "100%", padding: 14, borderRadius: 40, fontSize: 15 }),
            }}
          >
            Enter Dashboard →
          </button>
    ...
    >
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setManualForm({ ...manualForm, customer_email: e.target.value })
                }
                style={inp()}
              />
              <input
                ...
                onChange={(e) =>
                  setManualForm({ ...manualForm, customer_phone: e.target.value })
                }
                style={inp()}
              />
              <input
                ...
                onChange={(e) =>
                  setManualForm({ ...manualForm, delivery_address: e.target.value })
                }
                style={inp()}
              />
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                <button
                  ...
                  disabled={saving}
                  style={btn({ flex: 1, padding: "12px", borderRadius: 12 })}
                >
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Find<span style={{ color: G }}>Me</span>
                </h1>
            ...
            >
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <p style={{ margin: 0, fontSize: 13 }}>
                  Status:{" "}
                  <strong>
                    {(invoiceOrder.payment_status || "unpaid").toUpperCase()}
                  </strong>
                </p>
                <p style={{ margin: "2px 0 0", fontSize: 13 }}>
                  Method:{" "}
                  <strong>
                    {(invoiceOrder.payment_method || "Online")
                      .replace("_", " ")
                      .toUpperCase()}
                  </strong>
                </p>
            ...
            >
              <thead>
                <tr style={{ background: "#f8f9fa", textAlign: "left" }}>
                  <th style={{ padding: "10px", fontSize: 12, color: "#555" }}>
                    Item
                  </th>
                  <th
                    style={{
            ...
            >
              <div style={{ width: 220 }}>
                ...
                >
                  <span>Total</span>
                  <span>₦{(invoiceOrder.total || 0).toLocaleString()}</span>
                </div>
              </div>
            </div>
          ...
          >
            ...
            >
              <div>
                ...
                <p style={{ margin: 0, color: "#4a7a5a", fontSize: 13 }}>
                ...
                {selected.paid_at && (
                  ...
                  >
                    Paid on {new Date(selected.paid_at).toLocaleString()}
                  </p>
                )}
              </div>
              <button
            ...
            {(selected.items || []).map((item: any, i: number) => (
              ...
              >
                <span>
                  {item.name}{" "}
                  <span style={{ color: "#4a7a5a" }}>x{item.quantity}</span>
                </span>
            ...
            >
              ...
              >
                {selected.delivery_method === "pickup" ? "Pickup" : "Delivery"}
              </p>
              ...
              {selected.delivery_method === "pickup" && selected.pickup_code && (
                ...
                >
                  <span style={{ color: "#4a7a5a", fontSize: 12 }}>
                    Pickup Code:
                  </span>
                  <span
                    style={{
              ...
              {selected.ready_notified_at && (
                <p style={{ margin: "8px 0 0", color: G, fontSize: 12 }}>
                  ✅ Ready email sent{" "}
                  {new Date(selected.ready_notified_at).toLocaleString()}
                </p>
              )}
            </div>
            <p
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                ...
                <div key={f.key}>
                  ...
                  <input
                    value={String(editProduct[f.key] ?? "")}
                    onChange={(e) =>
                      setEditProduct({
                        ...editProduct,
                        [f.key]:
              ...
              <div style={{ display: "flex", gap: 12 }}>
                ...
                >
                  <input
                    type="checkbox"
                    checked={editProduct.coming_soon}
                    onChange={(e) =>
                      setEditProduct({
                        ...editProduct,
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        >
          ...
          >
            <h1 style={{ fontWeight: 800, fontSize: 18, margin: 0 }}>
              Find<span style={{ color: G }}>Me</span>
            </h1>
            <p
              style={{
            ...
            {tabs.map((t, i) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background:
                    tab === i ? "rgba(29,185,84,0.12)" : "transparent",
          ...
          <div style={{ padding: "0 10px" }}>
            <button
              onClick={() => {
                ...
                sessionStorage.removeItem("fm_admin_pass");
                setAuthed(false);
              }}
              style={{
                width: "100%",
        ...
        <div style={{ flex: 1, padding: "36px 36px", overflowY: "auto" }}>
          ...
          {tab === 0 && (
            <div>
              ...
              >
                <div>
                  <h2
                    style={{ fontWeight: 800, fontSize: 26, margin: "0 0 6px" }}
                  >
                    Overview
                  </h2>
                  <p style={{ color: "#4a7a5a", margin: 0, fontSize: 13 }}>
              ...
              >
                {[
                  ...
                  {
                    label: "Unpaid Orders",
                    value: orders.filter((o) => o.payment_status !== "paid")
                      .length,
                    icon: "⏳",
                    color: "#f87171",
                  },
                  ...
                  <div key={i} style={card()}>
                    ...
                    <p style={{ color: "#4a7a5a", fontSize: 12, margin: 0 }}>
                      {s.label}
                    </p>
                  </div>
                ))}
              ...
              {orders.slice(0, 5).map((o) => (
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span style={{ fontWeight: 800, color: G, fontSize: 14 }}>
                      ₦{(o.total || o.subtotal || 0).toLocaleString()}
                    </span>
                    ...
                    <span
                      style={{
                        background: STATUS_COLORS[o.status] || "#4a7a5a",
                        color: "#000",
                        padding: "3px 10px",
                        borderRadius: 20,
                        fontSize: 11,
          ...
          {tab === 1 && (
            <div>
              ...
              >
                <div>
                  <h2 style={{ fontWeight: 800, fontSize: 26, margin: "0 0 3px" }}>
                    Orders
                  </h2>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <select
                    ...
                    style={{
                      ...inp({ width: "auto", borderRadius: 40, cursor: "pointer" }),
                    }}
                  >
              ...
              {loading ? (
                <p style={{ color: "#4a7a5a", textAlign: "center", padding: 40 }}>
                  Loading...
                </p>
              ) : (
                filtered.map((o) => (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span
                        style={{
                          background: STATUS_COLORS[o.status || "pending"],
                          color: "#000",
                          padding: "4px 12px",
                          borderRadius: 20,
                          fontSize: 11,
          ...
          {tab === 2 && (
            <div>
              ...
              >
                <div>
                  <h2 style={{ fontWeight: 800, fontSize: 26, margin: "0 0 4px" }}>
                    Customers
                  </h2>
                  <p style={{ color: "#4a7a5a", fontSize: 13, margin: 0 }}>
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      <p style={{ color: "#4a7a5a", fontSize: 13, margin: 0 }}>
                        Send to all {customers.length} customers via WhatsApp one
                        by one.
                      </p>
                    </div>
                    <button
                          ...
                          .map((cp) => (
                            <button
                              ...
                              style={{
                                ...
                                fontSize: 13,
                                background:
                                  broadcastCoupon?.id === cp.id
                                    ? G
                                    : "rgba(29,185,84,0.1)",
                                color:
                                  broadcastCoupon?.id === cp.id ? "#000" : G,
                              }}
                            >
                  ...
                  {broadcastCoupon && !broadcastDone && (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      >
                        <p style={{ fontSize: 13, color: "#4a7a5a", margin: 0 }}>
                          Progress: {broadcastIdx} / {customers.length}
                        </p>
                        <p
                          style={{
                            fontSize: 13,
                        ...
                        >
                          {Math.round((broadcastIdx / customers.length) * 100)}%
                        </p>
                      </div>
                      <div
                        style={{
                      ...
                      >
                        <div
                          style={{
                            background: G,
                            height: "100%",
                            width: `${(broadcastIdx / customers.length) * 100}%`,
                            borderRadius: 40,
                          }}
                        />
                      </div>
                  ...
                  {broadcastDone && (
                    ...
                    >
                      ...
                      >
                        ✅ Broadcast complete! Sent to {customers.length}{" "}
                        customers.
                      </p>
                    </div>
                  )}
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <button
                        onClick={() => {
                          ...
                          let phone = (customer.customer_phone || "").replace(
                            /\D/g,
                            ""
                          );
                          if (phone.startsWith("0")) phone = "234" + phone.slice(1);
                          ...
                          const msg =
                            ...
                            "* — " +
                            broadcastCoupon.discount +
                            "% off your next FindMe order\n\nShop now: https://findme.com.ng/shop";
                          window.open(
                            "https://api.whatsapp.com/send?phone=" +
                              phone +
                              "&text=" +
                      ...
                      >
                        {broadcastIdx === 0
                          ? "▶ Start Broadcast"
                          : `💬 Send to ${customers[broadcastIdx]?.customer_name} (${broadcastIdx + 1}/${customers.length})`}
                      </button>
                      {broadcastIdx > 0 && (
                        <button
                          onClick={() => {
              ...
              {customers.map((c, i) => {
                const co = orders.filter(
                  (o) => o.customer_email === c.customer_email
                );
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p
                          style={{ color: "#4a7a5a", fontSize: 12, margin: 0 }}
                        >
                    ...
                    >
                      <div style={{ textAlign: "right" }}>
                        ...
                        <p
                          style={{ color: "#4a7a5a", fontSize: 11, margin: 0 }}
                        >
                          {co.length} order{co.length !== 1 ? "s" : ""}
                        </p>
                      </div>
                      <div style={{ position: "relative" }}>
                        <button
                          onClick={() =>
                            setOpenCouponMenu(openCouponMenu === i ? null : i)
                          }
                          style={{
                            padding: "7px 14px",
                            background: "rgba(29,185,84,0.12)",
                            ...
                            {coupons
                              ...
                              .map((cp) => (
                                ...
                                >
                                  ...
                                  <span
                                    style={{ color: "#4a7a5a", fontSize: 12 }}
                                  >
                                    {cp.discount}% off
                                  </span>
                                </button>
          ...
          {tab === 3 && (
            <div>
              <h2 style={{ fontWeight: 800, fontSize: 26, margin: "0 0 6px" }}>
                🎨 CMS
              </h2>
              <p style={{ color: "#4a7a5a", marginBottom: 28, fontSize: 13 }}>
              ...
              <div style={card()}>
                ...
                <p
                  style={{ color: "#4a7a5a", fontSize: 12, margin: "0 0 12px" }}
                >
                  Shown in the "Order Ready" email for self-pickup orders.
                </p>
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    defaultValue={cmsVal("pickup_location")}
                    id="cms-pickup_location"
                    style={inp()}
                    placeholder="e.g. FindMe Office, 12 Example Street, Lagos"
                  />
                  <button
                    onClick={() => {
                      const el = document.getElementById(
                      ...
                      if (el) saveCms("pickup_location", el.value);
                    }}
                    style={btn()}
                  >
                    Save
          ...
          {tab === 4 && (
            <div>
              ...
              <p style={{ color: "#4a7a5a", marginBottom: 24, fontSize: 13 }}>
              ...
              {products.length === 0 ? (
                ...
                >
                  <p style={{ color: "#4a7a5a", fontSize: 14 }}>
                    No products found. Run the SQL to insert your products.
                  </p>
                </div>
              ) : (
                products.map((p) => (
          ...
          {tab === 5 && (
            <div>
              ...
              <p style={{ color: "#4a7a5a", marginBottom: 24, fontSize: 13 }}>
                Edit customer testimonials on the homepage.
              </p>
              {testimonials.length === 0 ? (
                <div style={{ ...card(), textAlign: "center", padding: 40 }}>
                  <p style={{ color: "#4a7a5a" }}>No testimonials yet.</p>
                </div>
              ) : (
                testimonials.map((t) =>
                    ...
                    <div key={t.id} style={{ ...card(), marginBottom: 8 }}>
                      ...
                      >
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={editTestimonial.active}
                            onChange={(e) =>
                              setEditTestimonial({
                                ...editTestimonial,
                                active: e.target.checked,
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button
                            onClick={() => saveTestimonial(editTestimonial)}
                            style={btn({ flex: 1, borderRadius: 10 })}
                          >
                            Save
                          </button>
                          <button
          ...
          {tab === 6 && (
            <div>
              ...
              <p style={{ color: "#4a7a5a", marginBottom: 24, fontSize: 13 }}>
                Edit FAQs shown on the FAQ page.
              </p>
              {faqs.length === 0 ? (
                <div style={{ ...card(), textAlign: "center", padding: 40 }}>
                  <p style={{ color: "#4a7a5a" }}>No FAQs yet.</p>
                </div>
              ) : (
                faqs.map((f) =>
                  editFaq?.id === f.id ? (
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        ...
                        <p
                          style={{ color: "#4a7a5a", fontSize: 13, margin: 0 }}
                        >
              ...
              >
                <div>
                  ...
                  <p style={{ color: "#4a7a5a", fontSize: 13, margin: 0 }}>
                    ...
                    {coupons.reduce((s, c) => s + (c.used_count || 0), 0)} total
                    uses
                  </p>
                </div>
                <button
          ...
          {tab === 8 && (
            <div>
              ...
              <p style={{ color: "#4a7a5a", marginBottom: 28, fontSize: 13 }}>
                Generate QR codes and export a CSV for your printing company.
              </p>
              <div
                style={{
                  background: "rgba(29,185,84,0.06)",
              ...
              >
                ...
                >
                  📋 How to use with your printer
                </p>
                {[
                  "1. Enter how many QR codes you need and click Generate",
                  "2. Download the CSV file",
                  "3. Send the CSV + your tag design to the printing company",
                  "4. They use Variable Data Printing (VDP) to print each unique QR",
                  "5. Each QR links to https://app.findme.com.ng/scan/[code]",
                ].map((t, i) => (
                  <p
                    key={i}
                    style={{ margin: "0 0 6px", fontSize: 13, color: "#4a7a5a" }}
                  >
                    {t}
                  </p>
                ))}
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      padding: "12px 28px",
                      background: printLoading ? "#4a7a5a" : G,
                      color: "#000",
                      border: "none",
                      borderRadius: 12,
                      fontFamily: "Syne, sans-serif",
                ...
                {printLoading && (
                  <p style={{ marginTop: 12, fontSize: 13, color: G }}>
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {printDone.count} QR codes generated!
                      </p>
                      <p style={{ color: "#4a7a5a", fontSize: 13, margin: 0 }}>
                  ...
                  >
                    <p style={{ margin: "0 0 4px", color: G, fontWeight: 700 }}>
                      CSV Preview:
                    </p>
                    {printDone.csv
                      .split("\n")
          ...
          {tab === 9 && (
            <div>
              ...
              <p style={{ color: "#4a7a5a", marginBottom: 28, fontSize: 13 }}>
                Global configuration for FindMe
              </p>
              <div
                style={{
                  maxWidth: 560,
              ...
              >
                {[
                  ...
                  {
                    label: "Discount Code",
                    desc: "Sent in confirmation emails",
                    key: "discount_code",
                    icon: "🎟️",
                    val: discCode,
                    set: setDiscCode,
                  ...
                  <div key={i} style={card()}>
                    ...
                    >
                      ...
                      <div>
                        <p
                          style={{ fontWeight: 700, fontSize: 15, margin: 0 }}
                        >
                          {s.label}
                        </p>
                        <p
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button
                        onClick={() => {
                          saveCms(s.key, s.val);
                          showToast(s.label + " saved");
                        }}
                        style={btn({ borderRadius: 10 })}
                      >
                        Save
                      </button>
                    </div>
